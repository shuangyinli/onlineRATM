...
struct onlineModel {
	...
	int onlineG0;
	onlineModel(int onlinenum_docs_, int onlinenum_words_, int onlinenum_topics_, int onlinewin_, int onlineG0_, onlineModel* init_model = NULL){
		onlinenum_docs = onlinenum_docs_;
		onlinenum_words = onlinenum_words_;
		onlinewin = onlinewin_;
		onlinenum_topics = onlinenum_topics_;
		onlineG0 = onlineG0_;
		onlinepi = new double[onlinewin];
		onlinelog_beta = new double[onlinenum_topics * onlinenum_words];
		init(init_model);
	}
	void init(onlineModel* init_model=NULL);
	onlineModel(char* model_root, char* prefix) {
	        read_model_info(model_root);
	        char pi_file[1000];
	        sprintf(pi_file, "%s/%s.onlinepi", model_root, prefix);
	        char beta_file[1000];
	        sprintf(beta_file,"%s/%s.onlinetopicdis_overwords_beta",model_root,prefix);
	        //char alpha_file[1000];
	        //sprintf(alpha_file,"%s/%s.alpha",model_root,prefix);
	        onlinepi = load_mat(pi_file, onlinewin, 1);
	        onlinelog_beta = load_mat(beta_file, onlinenum_topics, onlinenum_words);
	    }
	    ~onlineModel() {
	        if (onlinepi)delete[] onlinepi;
	        if (onlinelog_beta) delete[] onlinelog_beta;
	    }
	    void read_model_info(char* model_root);
	    double* load_mat(char* filename,int row,int col);
};
